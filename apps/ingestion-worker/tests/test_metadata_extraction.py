...
@pytest.mark.parametrize("doc_mock, result_mock, file_path, expected_meta", [
    # Case 1: Metadata present in doc object
    (
        MagicMock(metadata=MagicMock(title="Doc Title", authors=["Alice"], creation_date="2023-01-01", language="fr"), num_pages=5),
        MagicMock(),
        "/tmp/test.pdf",
        {"title": "Doc Title", "author": "Alice", "created_at": "2023-01-01", "pages": 5, "language": "fr"}
    ),
    # Case 2: Metadata missing, fallback to filename and result pages
    (
        MagicMock(metadata=None, origin=MagicMock(filename="origin.pdf"), num_pages=None),
        MagicMock(pages=[1, 2, 3]),
        "/tmp/fallback.pdf",
        {"title": "origin.pdf", "author": None, "created_at": None, "pages": 3, "language": "en"}
    ),
    ...
    (
        ...
        MagicMock(pages=[]),
        "/path/to/base.pdf",
        {"title": "base.pdf", "author": None, "created_at": None, "pages": 0, "language": "en"}
    ),
    # Case 4: Authors is a list
    (
        MagicMock(metadata=MagicMock(title="T", authors=["Alice", "Bob"], creation_date=None, language="en"), num_pages=1),
        MagicMock(),
        "f.pdf",
        {"title": "T", "author": "Alice, Bob", "created_at": None, "pages": 1, "language": "en"}
    ),
    ...
    (
        ...
        MagicMock(),
        "c.pdf",
        {"title": "Callable Title", "author": "Callable Author", "created_at": "2024-01-01", "pages": 10, "language": "de"}
    ),
])
def test_file_metadata_extraction_logic(doc_mock, result_mock, file_path, expected_meta):
...
@pytest.mark.parametrize("crawl_result_mock, url, expected_meta", [
    # Case 1: Standard markdown title and links
    (
        MagicMock(
            markdown="# Web Title\nSome content", 
            links={'internal': [{'href': 'http://e.com/1'}, {'href': 'http://e.com/2'}], 'external': []},
            url="http://e.com/page"
        ),
        "http://e.com/page",
    ...
    (
        ...
        "http://e.com/",
        {"title": "Index", "path": "", "links_count": 1} # Only internal 'subpage' -> 'http://e.com/subpage'
    ),
])
