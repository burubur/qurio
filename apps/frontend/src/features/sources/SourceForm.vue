    ...
    <form @submit.prevent="submit" class="p-4 space-y-4">
      ...
      <div v-else class="space-y-4">
         <div class="grid w-full gap-1.5">
            ...
            <Input type="file" @change="onFileChange" accept=".pdf,.md,.txt,.html" class="cursor-pointer" />
            <p class="text-[0.8rem] text-muted-foreground">Supported: PDF, Markdown, Text, HTML (Max 50MB)</p>
         </div>
         <Button type="submit" :disabled="store.isLoading || !file" class="w-full sm:w-auto">
           <Loader2 v-if="store.isLoading" class="mr-2 h-4 w-4 animate-spin" />
           <Plus v-else class="mr-2 h-4 w-4" />
           Upload & Ingest
         </Button>
      ...
      <div v-if="store.error" class="text-destructive text-sm font-mono flex items-center gap-2 pt-2 border-t">
        <span>!</span>
        {{ store.error }}
      </div>
    </form>
