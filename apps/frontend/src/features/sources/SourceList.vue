...
<template>
  <div class="space-y-4">
    <div v-if="store.isLoading && store.sources.length === 0" class="text-center p-8 text-muted-foreground border border-dashed rounded-lg bg-muted/10">
      <div class="animate-spin h-6 w-6 border-2 border-primary border-t-transparent rounded-full mx-auto mb-4"></div>
      <span>Retrieving knowledge sources...</span>
    </div>
    
    <div v-else-if="store.sources.length === 0" class="text-center p-8 text-muted-foreground border border-dashed rounded-lg bg-muted/10">
    ...
    <div v-else class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      <Card v-for="source in store.sources" :key="source.id" class="bg-card">
        ...
        <CardFooter class="flex justify-end gap-2">
           ...
           <Button variant="ghost" size="icon" class="text-destructive hover:text-destructive hover:bg-destructive/10" @click="handleDelete(source.id)" title="Delete">
              <Trash2 :size="16" />
           </Button>
        </CardFooter>
      </Card>
